...
export const Media = memo(
   function Media({
      ...
      const duplicatedCarousel = useMemo(() => {
      ...
      const trailerUrl = useMemo(() => {
         ...
         return `https://www.youtube-nocookie.com/embed/${trailerKey}?autoplay=1&mute=${
            isMuted ? 1 : 0
      ...
      return (
         <div
            onClick={onClick}
            className="relative aspect-5/3 w-full overflow-hidden border border-zinc-800 bg-zinc-900 cursor-pointer select-none"
         ...
         >
            ...
            {isHoverSupported && duplicatedCarousel.length > 0 && (
               <div
               ...
               >
                  <div className="flex flex-col gap-2 animate-vertical-marquee will-change-transform transform-gpu">
                     {duplicatedCarousel.map((imgPath, idx) => (
                        <div
                           key={`${imgPath}-${idx}`}
            ...
            {isHoverSupported && trailerKey && (
               <>
                  ...
                  <button
                     type="button"
                     onClick={(e) => {
                        e.stopPropagation(); // Stop event from opening the modal
                        toggleMute(e);
                     }}
