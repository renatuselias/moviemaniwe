...
type MovieExtras = Awaited<ReturnType<typeof getMediaExtras>>;
   ...
   return (
      ...
      >
         {!extrasLoading &&
            (logo_path ? (
               <div
                  className={`mb-3 sm:mb-6 lg:mb-8 origin-bottom-left ${isImageLoading ? "hidden" : "block"}`}
               >
                  <Link
                     href={`/movie/${movieId}`}
                  ...
                  >
                     <Image
                        ...
                        className={`select-none w-64 sm:w-80 md:w-100 h-auto object-contain drop-shadow-[0_4px_20px_rgba(0,0,0,0.8)] ${
                           isVertical ? "max-w-40 sm:max-w-60" : "w-64 sm:w-80"
                        }`}
                     />
                  </Link>
               </div>
         ...
         {extrasLoading ? (
            ...
            <div className="flex flex-col gap-2 text-white/50 ">
               ...
               {/* genres */}
               {genres && (
                  <ul className="flex flex-wrap items-center gap-2 text-white/70 tracking-tighter cursor-pointer text-[14px]">
                     {genres.map((genre: { id: number }, index: number) => (
                        ...
                        >
                           {index > 0 && (
                              <span className="text-white/30 -mt-0.5 select-none text-3xl">
                                 •
                              </span>
                           )}
               ...
               <div className="flex items-center tracking-tighter gap-2 mb-3 sm:mb-4 font-medium drop-shadow-md text-sm cursor-default">
                  ...
                  {mediaType === "tv" && numberOfSeasons ? (
                     <>
                        <span className="text-white/20">|</span>
                        <span>{`${numberOfSeasons} ${t("mediaDetail.season")}`}</span>
                     </>
                  ) : null}
